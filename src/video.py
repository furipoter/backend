...
def video_upload():
    try:
        ...
        with create_runner("yolov7_i8_2pe.enf", worker_num=8) as runner:
            out = cv2.VideoWriter(video_no_audio_url, fourcc, fps, (new_width, new_height))
            cnt = 0
            while True:
                ret, _frame = video.read()
                if ret:
                    frame = cv2.resize(_frame, dsize=(new_width, new_height), interpolation=cv2.INTER_LINEAR)
